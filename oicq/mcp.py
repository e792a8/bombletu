...
`[:refer 消息ID]` 引用某条消息，或用于对特定消息发表回复。此元素每条消息中只能使用最多1次，且应放在消息开头。使用`get_messages_by_id`工具查阅消息ID对应的消息内容及其上下文。使用`get_messages`工具的`with_id`参数查询消息ID。你在一般浏览消息记录时无需使用with_id参数，减小信息量。
